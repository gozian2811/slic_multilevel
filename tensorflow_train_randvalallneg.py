...
net_store_name = "luna_slh_3D_bndo_flbias_l6_40_aug2"
net_store_path = store_path + "/" + net_store_name
tensorboard_path = net_store_path + "/tensorboard/"
#net_init_name = "luna_slh_3D_bndo_flbias_l6_40_aug_init"
...
num_positive_aug = num_positive * aug_proportion
...
net_outs, _, bn_params = tft.volume_bndo_flbias_l6_40(volume_reshape, True, positive_confidence, dropout_rate=0.0, batch_normalization_statistic=True, bn_params=None)
cross_entropy = tf.nn.softmax_cross_entropy_with_logits(logits = net_outs['last_out'], labels = real_label)
balancing = real_label[:,0] + tf.pow(tf.constant(-1, dtype=tf.float32), real_label[:,0]) * tf.constant(positive_confidence, tf.float32)
...
'''
epochs = [10, 0, 0]
learning_rates = [0.2, 0.001, 0.0001]
trains = []
		...
		for epoch in range(epochs[train_stage]):
			...
			for pbi in tqdm(range(0, positive_train_num, positive_batch_size)):
				...
				for bi in range(0, train_data.shape[0], BATCH_SIZE):
					...
					_, losses, accuracies, bn_stat, train_summary = sess.run([train, focal_loss, correct_prediction, bn_params, train_merge], {volume_input: data_batch, real_label: label_batch})
			...
			for pbi in tqdm(range(0, positive_val_num, positive_batch_size)):
				...
				for bi in range(0, val_data.shape[0], BATCH_SIZE):
					...
					losses, accuracies, bn_stat, val_summary = sess.run(
						[focal_loss, correct_prediction, bn_params, val_merge],
