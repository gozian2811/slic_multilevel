...
net_store_name = "luna_3D_l3454-512-2_bn_test"
#net_init_file = "models_tensorflow/luna_3D_l3454-512-2_bn/luna_3D_l3454-512-2_bn_epoch50"
...
#pfiles.extend(glob.glob(os.path.join(data_dir2,"npy","*.npy")))
...
ptfiles = pfiles[0:positive_train_num]
pvfiles = pfiles[positive_train_num:]
...
out_fc2, softmax_out = tft.volume_bnnet3_l6_56(volume_reshape)
	...
	for train_stage in range(len(trains)):
		...
		for epoch in range(epochs[train_stage]):
			...
			for pbi in range(0, positive_val_num, positive_batch_size):
				...
				#positive_data = np.load(pfiles[data_index])
