...
MAX_BOUND = constants["MAX_BOUND"]
MIN_BOUND = constants["MIN_BOUND"]
...
net_store_name = "luna_3D_l3435-512-2_do_aug"
net_init_file = "models_tensorflow/luna-3D-huge/luna-3D-huge_stage1"
data_dir1 = "luna_cubes_56_overbound"
data_dir2 = "SLH_cubes_56"
...
with tf.Session(config=config) as sess:
	...
	for train_stage in range(len(trains)):
		...
		for epoch in range(epochs[train_stage]):
			...
			train_loss = 0.0
			train_accuracy = 0.0
			positive_batch_size = (1-int(AUGMENTATION))*(BATCH_SIZE/2-1) + 1	#if augmentatiion implemented then set batch size to 1 or half of BATCH_SIZE
			for pbi in range(0, positive_train_num, positive_batch_size):
				...
				for pti in range(pbi, min(pbi+positive_batch_size, positive_train_num)):
					...
					patient_uid, nodule_diameter = mt.get_annotation_informations(pfile, "LUNA16/csvfiles/annotations.csv")
					positive_data = np.load(pfile)
					if "positive_batch" not in dir():
						positive_batch = mt.extract_volumes(positive_data, nodule_diameter=nodule_diameter, translation_augment=AUGMENTATION, rotation_augment=AUGMENTATIONe)
					else:
						positive_batch = np.concatenate((positive_batch, mt.extract_volumes(positive_data, nodule_diameter=nodule_diameter, translation_augment=AUGMENTATION, rotation_augment=AUGMENTATION)), axis=0)
				...
				for bi in range(0, train_data.shape[0], BATCH_SIZE):
					...
					_, predictions, losses, accuracies, summary = sess.run([train, out_fc2, cross_entropy, correct_prediction, merged], {volume_input: data_batch, real_label: label_batch})
			...
			for pbi in range(0, positive_val_num, positive_batch_size):
				...
				for pvi in range(pbi, min(pbi+positive_batch_size, positive_val_num)):
					...
					positive_data = np.load(pfile)
					if "positive_batch" not in dir():
						positive_batch = mt.extract_volumes(positive_data, nodule_diameter=nodule_diameter)
					else:
						positive_batch = np.concatenate((positive_batch, mt.extract_volumes(positive_data, nodule_diameter=nodule_diameter)), axis=0)
				#print("validation process:%d/%d" % (pvi+1, positive_val_num))
				#data_index = positive_indices[positive_train_num+pvi]
				#positive_data = np.load(tpfiles[data_index])
				#positive_batch = mt.extract_volumes(positive_data, augment=False)
				negative_batch_size = min(positive_batch.shape[0], negative_probabilities.nonzero()[0].size)
				...
				#fusion_batch_size = positive_batch.shape[0] + val_portion_size
				for ni in range(negative_candidate.size):
					negative_batch[ni] = np.load(nfiles[negative_candidate[ni]])
					negative_probabilities[negative_candidate[ni]] = 0
					#if negative_unused.count(negative_candidate[ni])>0:
					#	negative_probabilities = np.delete(negative_probabilities, negative_candidate[ni])
					#	negative_unused.remove(negative_candidate[ni])
				fusion_batch_size = positive_batch.shape[0] + negative_batch.shape[0]
				...
				for bi in range(0, val_data.shape[0], BATCH_SIZE):
					...
					losses, accuracies, summary = sess.run([cross_entropy, correct_prediction, merged], {volume_input: data_batch, real_label: label_batch})
					for di in range(data_batch.shape[0]):
			...
			#if (train_stage*DECAY_EPOCH+epoch == 0) or negative_importances.nonzero()[0].size<=0:
			if negative_importances.nonzero()[0].size<=0:
			...
			logger.close()
			if (overall_epoch+1)%SNAPSHOT_EPOCH==0:
...
sess.close()
negative_losses_output = open("results/negative_losses.txt", "w")
